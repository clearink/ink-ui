...
async function build(options) {
  ...
  {
    const spinner = ora(logger.info('starting clean dist\n', false)).start();
    await Promise.all([!options.onlyGenIcons && fse.remove(constants.esm), !options.onlyGenIcons && fse.remove(constants.cjs), !options.onlyGenIcons && fse.remove(constants.umd), fse.remove(constants.resolveSrc('icons'))]);
    spinner.succeed(logger.success('clean dist successfully !\n', false));
    spinner.clear();
  }
  {
...
program.command('build:icons').description('build icon library').option('--only-gen-icons', 'only generate icons', false).action(build);
